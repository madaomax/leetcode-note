...
Quickselect, also known as Hoare's selection algorithm, is an algorithm for finding kth smallest element in an unordered list. It is significant because it has an average runtime of O(n). [Wow!]

Uses the same idea as quicksort.

1. Choose a pivot index. Usually chosen randomly.
