...
If left half `[low...mid]` is sorted, we know this if the element at `low` <= the element  at `mid`. In any sorted array, if the start if less than or equal to the midpoint, it means all the elements till the midpoint are in increasing order.
